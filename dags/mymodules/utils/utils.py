...
from mymodules.s3_utils.file_storage_layer import FileStorageOperations
    ...
    def decorator(
        ...
        def mytask(*airflow_inputss: dict[str, str]) -> dict[str, str]:
            ...
            for idx, (filename, content) in enumerate(func_outputs):
                ...
                airflow_outputs[s3folder_outputs[idx]] = filename
                s3_path = (
                    s3folder_outputs[idx]
                    + f"{run_id}/"
                    + airflow_outputs[s3folder_outputs[idx]]
                )
                if os.getenv("FILE_STORAGE_LAYER_ENV") == "local":
                    s3_path = f"data/s3/{s3_path}"
